---
title: "bulk_preprocessed_pipeline_0525"
author: "Kevin"
date: "2025-05-15"
output: pdf_document
---
...
preprocess_seed_data <- function(file_path, plot = FALSE, save_output = TRUE) {
  ...
  if (length(spectral_cols) == 0) {
    stop("❌ No valid spectral columns found in file: ", basename(file_path))
  }

  message("🧪 Found ", length(spectral_cols), " spectral columns.")
  ...
  mutate(
    ...
    # Generate plots
    plot_ribbon(df_long, "reflectance", "Raw Reflectance", "Reflectance", "bulk_plot_raw.png")
    plot_ribbon(df_long, "norm_reflectance", "Normalized Reflectance", "Normalized Reflectance", "bulk_plot_norm_reflectance.png")
    plot_ribbon(df_long, "norm_reflectance_smooth", "Smoothed Reflectance", "Normalized Smoothed", "bulk_plot_norm_smooth.png")
    plot_ribbon(df_long, "norm_reflectance_deriv", "First Derivative", "Normalized Derivative", "bulk_plot_norm_derivative.png")

    message("📊 Saved 4 mean ± SD plots with expanded labels for: ", file_tag)
  }

  # ---- Save Wide Format CSV ----
  if (save_output) {
    if (!str_detect(file_path, "raw")) {
      output_wide <- str_replace(file_path, ".csv", "bulk_normalized_wide.csv")
    } else {
      output_wide <- str_replace(file_path, "raw", "bulk_normalized") %>%
    ...
    message("💾 Writing wide-format to: ", output_wide)
    tryCatch({
      write.csv(df_wide, output_wide, row.names = FALSE)
    }, error = function(e) {
      message("❌ Failed to write wide-format file: ", e$message)
    })
  }
...
safe_preprocess <- function(file_path) {
  if (!str_detect(file_path, "raw")) {
    output_wide <- str_replace(file_path, ".csv", "bulk_normalized_wide.csv")
    expected_plots <- c(
      str_replace(file_path, ".csv", "bulk_plot_raw.png"),
      str_replace(file_path, ".csv", "bulk_plot_norm_reflectance.png"),
      str_replace(file_path, ".csv", "bulk_plot_norm_smooth.png"),
      str_replace(file_path, ".csv", "bulk_plot_norm_derivative.png")
    )
  } else {
    output_wide <- str_replace(file_path, "raw", "bulk_normalized") %>%
      str_replace(".csv", "_wide.csv")
    tag <- tools::file_path_sans_ext(basename(file_path))
    expected_plots <- paste0(tag, c("bulk_plot_raw.png", "bulk_plot_norm_reflectance.png", "bulk_plot_norm_smooth.png", "_plot_norm_derivative.png"))
  }

  # Skip if wide file and all plots exist
  if (file.exists(output_wide) && all(file.exists(expected_plots))) {
    message("✅ Skipping (already processed): ", basename(file_path))
    return(NULL)
  }

  message("⚙️ Processing: ", basename(file_path))
  result <- preprocess_seed_data(file_path = file_path, plot = TRUE, save_output = TRUE)
  message("✅ Done: ", basename(file_path))
  return(result)
}
